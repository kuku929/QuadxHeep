...
module quadrilatero_rf_sequencer #(
  ...
  always_comb begin: ctrl_block
    ...
    for (int jj = 0; jj < READ_PORTS; jj++) begin: read_request
      ...
      // Indicate we are ready once the scoreboard looks good, but we wait for the grant to pop the entry and free the slot in the scoreboard
      // We do not want to pop until the system is ready to accept the data, but we still need to indicate that we are ready to accept the data.
      if( scoreboard_q[m][n].id  == rd_id_i[jj]     &&
          scoreboard_q[m][n].rvalid ) 
        begin
          rd_req [jj] = 1'b1;
      end
      if( scoreboard_q[m][n].id  == rd_id_i[jj]     &&
